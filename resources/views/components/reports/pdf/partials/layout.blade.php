...
<body>
    <div class="header">
        <h1><span class="accent">Attach</span>KE &mdash; {{ $title }}</h1>
        <p>Generated: {{ $generated_at ?? now()->format('d M Y, H:i') }} &bull; {{ $app_name ?? config('app.name', 'AttachKE') }}</p>
    </div>
    {{ $slot }}
    <div class="footer">
        <span>{{ $app_name ?? config('app.name', 'AttachKE') }} &copy; {{ date('Y') }}</span>
